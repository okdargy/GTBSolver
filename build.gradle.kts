...
tasks {
    ...
    val prismCopyEnabled = System.getenv("PRISM_COPY")?.lowercase() == "true"
    val prismModsPath = System.getenv("PRISM_MODS_PATH")

    remapJar {
        doLast {
            if (prismCopyEnabled) {
                if (prismModsPath != null) {
                    val jarFile = archiveFile.get().asFile
                    val target = File(prismModsPath, jarFile.name)

                    try {
                        jarFile.copyTo(target, overwrite = true)
                        logger.lifecycle("${jarFile.name} successfully copied to PRISM_MODS_PATH")
                    } catch (e: Exception) {
                        logger.error("Failed to copy jar to Prism mods path: ${e.message}")
                    }
                } else {
                    logger.warn("PRISM_MODS_PATH environment variable is not set.")
                }
            }
        }
    }
